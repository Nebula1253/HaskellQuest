...
-- MetaFractals form a larger meta-tree - some nodes contain 
-- Fractal trees
...
-- PLAYER 1 CODE
-- Dr Fractal's inner workings are a complex tree structure,
-- where some nodes contain Fractal trees that need to be
-- healed!
-- Write a function to traverse the MetaFractal structure:
-- if you encounter an UnstableMeta node, call player 2's
-- 'healFractal' function on the contained Fractal.

healMeta :: MetaFractal -> MetaFractal
healMeta meta = undefined 

-- hint: The recursive step needs to behave differently
-- depending on whether it's a Stable or UnstableMeta
